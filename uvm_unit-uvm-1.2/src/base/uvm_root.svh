...
// UVM_UNIT Modification
//
// For uvm_unit, we don't want const because uvm_root will be destroyed and recreated for each unit test.
//
...
function void uvm_root::report_header(UVM_FILE file = 0);
  ...
  // UVM_UNIT Modification
  //
  // For uvm_unit, don't print the header
  //
...
// `endif
  ...
  //////////////////////////////////////////////////////////
  // UVM_UNIT Modification
  end else begin // For uvm_unit we want to complain that no unit test was specified.
    $stacktrace;
    $display({"UVM_UNIT FATAL ERROR: run_test() must be called with a child ",
      "of uvm_unit_fixture specified."});
    $finish;
  end
  ...
  // uvm_unit will print it's own message
  // begin
  // 	if(test_name=="") 
  ...
  // UVM_UNIT Modification
  //
  // uvm_unit should never finish on completion.
...
task uvm_root::run_phase (uvm_phase phase);
  ...
  // We can't reset the simulator time to zero for each test.
  //
  // if($time > 0)
